...
class SP500APIManager {
    constructor() {
        ...
        this.sp500Symbols = [
            ...
            'UNH', 'META', 'XOM', 'LLY', 'JNJ', 'JPM', 'V', 'PG', 'MA', 'HD',
            'CVX', 'MRK', 'ABBV', 'PEP', 'KO', 'AVGO', 'PFE', 'BAC', 'TMO',
            'COST', 'DIS', 'ABT', 'WMT', 'CRM', 'DHR', 'VZ', 'NFLX', 'ADBE',
            'CMCSA', 'TXN', 'NKE', 'PM', 'RTX', 'NEE', 'WFC', 'COP', 'BMY',
            'UNP', 'T', 'SCHW', 'LOW', 'ORCL', 'HON'
        ];
    ...
    loadAPIKeys() {
        // 로컬 스토리지에서 API 키 로드
        this.apiKeys = {
            alphaVantage: localStorage.getItem('alpha_vantage_key') || process?.env?.ALPHA_VANTAGE_KEY,
            financialModelingPrep: localStorage.getItem('fmp_key') || process?.env?.FMP_KEY,
            twelveData: localStorage.getItem('twelve_data_key') || process?.env?.TWELVE_DATA_KEY,
            polygon: localStorage.getItem('polygon_key') || process?.env?.POLYGON_KEY,
            ...
            for (const symbol of symbols) {
                ...
                if (data.contents) {
                    ...
                    if (chartData.chart?.result?.[0]) {
                        ...
                        if (meta && quote) {
                            const lastPrice = meta.regularMarketPrice || quote.close?.slice(-1)[0];
                            ...
                            collectedData.push({
                                ...
                                changePercent: ((lastPrice - previousClose) / previousClose * 100).toFixed(2),
    ...
    integrateAndAnalyzeData() {
        ...
        for (const [symbol, sources] of integratedData) {
            if (sources.length > 0) {
                ...
                const consolidated = this.consolidateMultipleSourceData(symbol, sources);
    ...
    consolidateMultipleSourceData(symbol, sources) {
        ...
        return {
            ...
            change: totalWeight > 0 ? (weightedChange / totalWeight) : sources[0].change,
            changePercent: ((weightedChange / totalWeight) / (weightedPrice / totalWeight) * 100).toFixed(2),
            volume: totalVolume,
            marketCap,
    ...
    performMarketAnalysis(data) {
        ...
        // 평균 변동률
        const avgChange = data.reduce((sum, stock) => sum + (stock.change || 0), 0) / data.length;
    ...
    notifyDashboard() {
        ...
        // 커스텀 이벤트 발생
        window.dispatchEvent(new CustomEvent('sp500DataUpdate', {
            detail: {
