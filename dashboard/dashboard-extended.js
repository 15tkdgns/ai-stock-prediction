...
class DashboardExtensions {
    ...
    init() {
        console.log('[DASHBOARD EXTENSIONS DEBUG] DashboardExtensions init() started');
        console.log('[DASHBOARD EXTENSIONS DEBUG] Instance created successfully:', this);
        console.log('[DASHBOARD EXTENSIONS DEBUG] Dashboard reference:', this.dashboard);
        ...
        this.loadXAIData().then(() => {
            ...
            this.updateXAICharts(); // Call updateXAICharts instead of updateXAIExplanation
    ...
    updateChartDescription(stockSymbol) {
        ...
        if (description && stockSelector) {
            ...
            description.textContent = `Currently displaying real-time price prediction chart for ${companyName}. ` +
        ...
        if (shapCtx) {
            ...
            this.shapChart = new Chart(shapCtx, {
                ...
                options: {
                    ...
                    scales: {
                        x: { title: { display: true, text: 'SHAP Value (Impact on Prediction)' } },
                        y: { display: false }
                    }
                }
            });
    ...
    renderLocalXaiAnalysis(stockSymbol) {
        console.log(`[XAI DEBUG] Rendering individual XAI analysis for: ${stockSymbol}`);
        console.log(`[XAI DEBUG] Available XAI data:`, this.xaiData);

        if (!this.xaiData) {
            console.error(`[XAI DEBUG] No XAI data available, cannot render analysis for ${stockSymbol}`);
            return;
        }
        ...
        if (forcePlotContainer) {
            ...
            let positiveFeatures = [{name: '5-day Volume', value: 3.2}, {name: 'News Sentiment', value: 2.8}];
            let negativeFeatures = [{name: '20-day MA', value: -0.8}];
            
            if (this.xaiData && this.xaiData.explainability && this.xaiData.explainability.shap_available) {
                console.log(`[XAI DEBUG] Using real SHAP data for ${stockSymbol}`);
                // Extract SHAP values from real data
                const shapData = this.xaiData.explainability.feature_importance_methods?.random_forest_builtin?.importance;
                if (shapData) {
                    ...
                    positiveFeatures = shapData.slice(0, 3).map((val, idx) => ({
                        name: `Feature_${idx + 1}`,
                        value: val * 10
                    })).filter(f => f.value > 0);
                    
                    negativeFeatures = shapData.slice(3, 6).map((val, idx) => ({
                        name: `Feature_${idx + 4}`,
                        value: val * 10
                    })).filter(f => f.value < 0);
                }
            } else {
                console.warn(`[XAI DEBUG] Using mock SHAP data for ${stockSymbol} - real data not available`);
            }
        ...
        if (limeContainer) {
            ...
            let limeExplanations = [
                { text: 'News: "New Product Launch"', contribution: 3.5, positive: true },
                { text: '10% Increase in Volume', contribution: 1.8, positive: true },
                { text: 'RSI exceeding 75', contribution: -2.1, positive: false }
            ];
            
            if (this.xaiData && this.xaiData.explainability && this.xaiData.explainability.lime_available) {
                console.log(`[XAI DEBUG] Using real LIME data for ${stockSymbol}`);
                // Generate dynamic LIME explanations based on real data
                const importance = this.xaiData.explainability.feature_importance_methods?.random_forest_builtin?.importance;
                if (importance) {
                ...
                console.warn(`[XAI DEBUG] Using mock LIME data for ${stockSymbol} - real data not available`);
            }
    ...
    async loadXAIData() {
        try {
            console.log('[XAI DEBUG] Attempting to load XAI data from monitoring_dashboard.json...');
            const response = await fetch('../data/raw/monitoring_dashboard.json');
            console.log('[XAI DEBUG] Fetch response status:', response.status, response.statusText);
            ...
            if (data && data.explainability) {
                this.xaiData = data;
                console.log('[XAI DEBUG] XAI data loaded successfully with explainability section');
                console.log('[XAI DEBUG] Available explainability features:', Object.keys(data.explainability));
                ...
                console.warn('[XAI DEBUG] XAI data is not in the correct format. Missing explainability section. Using mock data.');
                console.log('[XAI DEBUG] Available data keys:', data ? Object.keys(data) : 'No data');
                this.xaiData = this.generateMockXAIData();
                console.log('[XAI DEBUG] XAI data (mock) generated and assigned.', this.xaiData);
                this.updateXAICharts();
            }
        } catch (error) {
            ...
            this.xaiData = this.generateMockXAIData();
            console.log('[XAI DEBUG] XAI data (mock) generated and assigned after error.', this.xaiData);
            this.updateXAICharts();
        }
    }
    ...
    updateXAICharts() {
        ...
        console.log('[XAI DEBUG] Explainability section available:', !!this.xaiData?.explainability);
    ...
    generateMockXAIData() {
        ...
        return {
            explainability: {
                ...
                lime_explanations: [
                    {
                        explanation: [['news_sentiment', 0.05], ['price_change', 0.03], ['volume', -0.02]],
                        prediction_proba: [0.2, 0.8]
                    }
                ],
                partial_dependence_plots: {
                    'price_change': { x: [0, 0.01, 0.02, 0.03], y: [0.1, 0.3, 0.7, 0.9] },
                    'news_sentiment': { x: [-1, -0.5, 0, 0.5, 1], y: [0.2, 0.3, 0.5, 0.7, 0.8] }
                },
                counterfactual_what_if: [
                    { condition: 'news_sentiment was positive', result: 'prediction would be UP' },
                    { condition: 'volume was 20% higher', result: 'prediction would be DOWN' }
                ],
                prediction_data: {
                    sample_predictions: [
    ...
    updateXAICharts() {
        ...
        console.log('Rendering Feature Importance. Data available:', !!this.xaiData?.explainability?.feature_importance_methods);
        this.renderFeatureImportance();
        
        console.log('Rendering SHAP Summary Plot. Data available:', !!this.xaiData?.explainability?.shap_explanations);
        this.renderSHAPSummaryPlot();
        
        console.log('Rendering SHAP Dependence Plot. Data available:', !!this.xaiData?.explainability?.shap_explanations);
        this.renderSHAPDependencePlot();
        
        console.log('Rendering SHAP Force Plot. Data available:', !!this.xaiData?.explainability?.shap_explanations);
        this.renderSHAPForcePlot();
        
        console.log('Rendering LIME Explanation. Data available:', !!this.xaiData?.explainability?.lime_explanations);
        this.renderLIMEExplanation();
        
        console.log('Rendering Confusion Matrix. Data available:', !!this.xaiData?.model_performance?.confusion_matrix);
        this.renderConfusionMatrix();
        
        console.log('Rendering Partial Dependence Plot. Data available:', !!this.xaiData?.explainability?.partial_dependence_plots);
        this.renderPartialDependencePlot();
        
        console.log('Rendering Counterfactual What-if. Data available:', !!this.xaiData?.explainability?.counterfactual_what_if);
        this.renderCounterfactualWhatIf();
    ...
    renderFeatureImportance() {
        ...
        if (!container) {
            console.error('[XAI DEBUG] Could not find feature-importance-chart container.');
            return;
        }
        ...
        if (!importanceMethods) {
            console.warn('[XAI DEBUG] Feature importance data not found, showing error message');
            container.innerHTML = '<div class="xai-error"><p>Feature importance data not found. Check console for debugging info.</p></div>';
            return;
        }
        ...
        if (rfBuiltin && Array.isArray(rfBuiltin.importance) && rfBuiltin.importance.length > 0) {
            ...
            console.warn('[XAI DEBUG] Feature importance data format is incorrect, showing mock data');
            ...
            const mockFeatures = [
                ...
                { name: 'Moving Average', importance: 0.10 },
            ...
            html += '</div>';
            html += '<p class="xai-note"><em>Note: Displaying mock data as real feature importance data is not available.</em></p>';
            container.innerHTML = html;
        }
    }
    ...
    renderSHAPSummaryPlot() {
        ...
        if (!shapExplanations || !shapExplanations.random_forest_shap) {
            console.warn('[XAI DEBUG] SHAP Summary Plot data not found, showing mock visualization');
        ...
        if (rfShap.feature_names && rfShap.shap_values && rfShap.shap_values.length > 0) {
            const values = Array.isArray(rfShap.shap_values[0]) ? rfShap.shap_values[0] : rfShap.shap_values; // Use first prediction
        ...
        html += '</div>';
        html += '<div class="shap-legend"><span class="positive">▌ Positive Impact</span><span class="negative">▌ Negative Impact</span></div>';
        container.innerHTML = html;
    ...
    getFeatureDescription(featureName) {
        const descriptions = {
            ...
            'sma_20': '20-day Simple Moving Average - Average price over the last 20 days',
            'sma_50': '50-day Simple Moving Average - Average price over the last 50 days',
            ...
            'price_change': 'Price Change - Percentage change in price from the previous day',
            'volume_change': 'Volume Change - Percentage change in volume from the previous day',
            'unusual_volume': 'Unusual Volume - Whether trading volume is higher than average',
            'price_spike': 'Price Spike - Whether there was a sharp increase in price',
            'news_sentiment': 'News Sentiment - Degree of positive/negative sentiment in related news',
    ...
    renderSHAPDependencePlot() {
        console.log('Rendering: SHAP Dependence Plot (skipped - redundant with partial dependence plot)');
        ...
        if (!shapExplanations || !shapExplanations.random_forest_shap || !predictionData) {
            container.innerHTML = '<p>SHAP Dependence Plot data not found.</p>';
            return;
        }
        ...
        // Display explanation for the 3 most important features
        ...
        let html = '<h4>SHAP Dependence Analysis</h4>';
        html += '<p>Impact of most important features on prediction:</p>';
        html += '<div class="dependence-analysis">';
        
        topFeatures.forEach((feature, index) => {
            const impact = feature.importance > 0.05 ? 'High' : feature.importance > 0.02 ? 'Medium' : 'Low';
            ...
            html += `
                <div class="dependence-item">
                    ...
                    <div class="feature-description">
                        Importance: ${(feature.importance * 100).toFixed(2)}% | Rank: ${index + 1}
                    </div>
                </div>
            `;
        });
    ...
    renderSHAPForcePlot() {
        console.log('Rendering: SHAP Force Plot');
        const container = document.getElementById('shap-force-plot');
        if (!container) return;

        const shapExplanations = this.xaiData?.explainability?.shap_explanations;
        if (!shapExplanations || !shapExplanations.random_forest_shap) {
            container.innerHTML = '<div class="xai-error"><p>SHAP Force Plot data not found.</p></div>';
            return;
        }
        ...
        // Force Plot summary for the first sample
        if (rfShap.shap_values && rfShap.shap_values.length > 0) {
            const sampleIndex = 0; // First sample
            // If shap_values is a 2D array, use the first element (SHAP values for class 1)
            ...
            let html = '<h4>SHAP Force Plot (First Sample)</h4>';
            html += '<p>Model\'s Base Prediction Value: <strong>' + baseValue.toFixed(4) + '</strong></p>';
            html += '<p>Contribution of each feature to the prediction value:</p>';
            ...
            const contributions = featureNames
                ...
                .filter(f => Math.abs(f.value) > 0.0001) // Filter values close to 0
                .sort((a, b) => Math.abs(b.value) - Math.abs(a.value));

            contributions.forEach(c => {
                const sign = c.value >= 0 ? 'Positive' : 'Negative';
    ...
    renderLIMEExplanation() {
        ...
        if (!limeExplanations || limeExplanations.length === 0) {
            container.innerHTML = '<div class="xai-error"><p>LIME explanation data not found.</p></div>';
            return;
        }
        ...
        if (firstLimeExplanation && firstLimeExplanation.explanation) {
            ...
            html += '<p>Prediction Probability: ' + (firstLimeExplanation.prediction_proba[1] || 0).toFixed(4) + '</p>';
            html += '<ul class="feature-importance-list">';
    ...
    renderConfusionMatrix() {
        ...
        if (!confusionMatrixData || !confusionMatrixData.matrix || !confusionMatrixData.labels) {
            container.innerHTML = '<div class="xai-error"><p>Confusion Matrix data not found.</p></div>';
            return;
        }
    ...
    renderPartialDependencePlot() {
        ...
        if (!pdpData) {
            container.innerHTML = '<div class="xai-error"><p>Partial Dependence Plot data not found.</p></div>';
            return;
        }

        let html = '<h4>Partial Dependence Plot</h4>';
        html += '<div class="partial-dependence-container">';
        html += '<p>Shows how model predictions change with feature value changes:</p>';
    ...
    renderCounterfactualWhatIf() {
        ...
        if (!whatIfData || whatIfData.length === 0) {
            container.innerHTML = '<div class="xai-error"><p>Counterfactual/What-if Analysis data not found.</p></div>';
            return;
        }
    ...
    async loadSP500Data() {
        try {
            ...
            for (let i = 1; i < lines.length && i <= 500; i++) {
                ...
                if (values.length >= 3) {
                    this.sp500Data.push({
                        ...
                        lastUpdate: new Date().toLocaleTimeString('en-US', { hour12: false }) + ' KST',
            ...
            console.error('S&P 500 data load failed:', error);
    ...
    createHeatmap(elementId, title, labels, data) {
        ...
        html += '<thead><tr><th></th>';
        labels.forEach(label => html += `<th>${label} (Predicted)</th>`);
        html += '</tr></thead><tbody>';
    ...
    async loadNewsData() {
        try {
            ...
            console.warn('News data load failed, using mock data:', error);
        ...
        const mockNews = [
            {
                ...
                content: 'The Federal Reserve raised its benchmark interest rate by 0.25 percentage points, signaling its commitment to curbing inflation. This has led to mixed reactions in the stock market, with technology stocks particularly affected.',
                sentiment: 'negative',
                sentiment_score: -0.3,
            ...
            },
            {
                ...
                content: 'Apple has unveiled its iPhone 15 series with innovative features. The adoption of a USB-C port and improved camera performance are drawing attention, and the stock price rose 3% after the announcement.',
                sentiment: 'positive',
                sentiment_score: 0.6,
                timestamp: new Date(Date.now() - Math.random() * 86400000).toISOString(),
                source: 'Bloomberg',
            ...
            },
            {
                ...
                content: 'Tesla announced that its Q3 revenue and net income significantly exceeded market expectations. Increased electric vehicle sales and growth in the energy storage business were key drivers.',
                sentiment: 'positive',
                sentiment_score: 0.8,
                timestamp: new Date(Date.now() - Math.random() * 86400000).toISOString(),
                source: 'CNBC',
            ...
            },
            {
                ...
                content: 'Microsoft announced a significant expansion of its investment in artificial intelligence and the integration of AI capabilities into its Azure cloud services. The partnership with OpenAI is also expected to be further strengthened.',
                sentiment: 'positive',
                sentiment_score: 0.5,
                timestamp: new Date(Date.now() - Math.random() * 86400000).toISOString(),
                source: 'TechCrunch',
            ...
            },
            {
                ...
                content: 'Due to persistent inflationary pressures, consumer goods companies\' stock prices have uniformly declined. Rising raw material costs and increased labor costs are identified as key factors.',
                sentiment: 'negative',
                sentiment_score: -0.4,
                timestamp: new Date(Date.now() - Math.random() * 86400000).toISOString(),
    ...
    updateNewsFeedDisplay() {
        ...
        if (this.newsCache.length === 0) {
            newsFeedContainer.innerHTML = '<div class="news-loading">Loading news...</div>';
            return;
        }
    ...
    createNewsItem(news) {
        ...
        const publishedDate = news.publishedAt ? new Date(news.publishedAt).toLocaleString('en-US') : 'No Date';
    ...
    getFeatureDescription(featureName) {
        const descriptions = {
            ...
            'price_change': 'Price change rate compared to the previous day',
            'rsi': 'Relative Strength Index (overbought/oversold judgment)',
            'macd': 'Moving Average Convergence Divergence indicator',
            'sma_20': 'Average stock price over the last 20 days',
            'sma_50': 'Average stock price over the last 50 days',
            'news_sentiment': 'Sentiment score of news articles',
            'bb_upper': 'Bollinger Band upper line',
            'bb_lower': 'Bollinger Band lower line',
            'atr': 'Average True Range indicator',
            'obv': 'Volume-based trend indicator'
        };
...
console.log('Hello from ${filePath}');`;
    ...
    renderMockSHAPSummary(container) {
        ...
        html += '</div>';
        html += '<div class="shap-legend"><span class="positive">▌ Positive Impact</span><span class="negative">▌ Negative Impact</span></div>';
        html += '<p class="xai-note"><em>Note: Displaying mock SHAP data as real explanations are not available.</em></p>';
    ...
    refreshXAIData() {
        ...
        containers.forEach(containerId => {
            ...
            if (container) {
                container.innerHTML = '<div class="xai-loading"><p>🔄 Refreshing analysis...</p></div>';
            }
        });
        ...
        this.loadXAIData().then(() => {
            ...
            if (stockSelector && stockSelector.value) {
                console.log('[XAI DEBUG] Refreshing stock analysis for:', stockSelector.value);
                this.renderLocalXaiAnalysis(stockSelector.value);
            }
            
            // Show success message
            if (this.dashboard && this.dashboard.showNotification) {
                this.dashboard.showNotification('XAI analysis refreshed successfully', 'success');
            }
        }).catch(error => {
            ...
            // Show error message
            if (this.dashboard && this.dashboard.showNotification) {
                this.dashboard.showNotification('Failed to refresh XAI analysis', 'error');
            }
    ...
    renderPerformanceMetricsChart() {
        ...
        const performanceData = {
            labels: ['Random Forest', 'Gradient Boosting', 'LSTM', 'Transformer', 'Ensemble'],
            datasets: [
                {
                    label: 'Accuracy (%)',
    ...
    renderLearningCurvesChart() {
        ...
        for (let i = 1; i <= 50; i++) {
            ...
            validationData.push(65 + 18 * (1 - Math.exp(-i/18)) + Math.random() * 3);
        }
    ...
    renderCorrelationHeatmap() {
        ...
        const features = ['Volume', 'Price_MA_5', 'Price_MA_20', 'RSI', 'MACD', 'News_Sentiment', 'VIX'];
        const correlationMatrix = [
            [1.00, 0.23, 0.18, -0.12, 0.34, 0.45, -0.67],
            [0.23, 1.00, 0.89, -0.34, 0.56, 0.12, -0.23],
            [0.18, 0.89, 1.00, -0.28, 0.48, 0.08, -0.19],
            [-0.12, -0.34, -0.28, 1.00, -0.45, -0.23, 0.34],
            [0.34, 0.56, 0.48, -0.45, 1.00, 0.37, -0.42],
            [0.45, 0.12, 0.08, -0.23, 0.37, 1.00, -0.56],
            [-0.67, -0.23, -0.19, 0.34, -0.42, -0.56, 1.00]
        ];
        ...
        html += '<div class="legend-item"><span class="legend-color positive"></span> Positive Correlation</div>';
        html += '<div class="legend-item"><span class="legend-color negative"></span> Negative Correlation</div>';
        html += '<div class="legend-note">Color intensity represents correlation strength</div>';
        html += '</div>';
    ...
    renderSentimentTimelineChart() {
        ...
        for (let i = 30; i >= 0; i--) {
            ...
            const baseSentiment = 0.1 + 0.4 * Math.sin(i * 0.2) + (Math.random() - 0.5) * 0.3;
    ...
    renderTopicModelingChart() {
        ...
        const topics = [
            { name: 'Market Volatility', weight: 0.28, keywords: ['volatility', 'uncertainty', 'risk'] },
            { name: 'Tech Earnings', weight: 0.22, keywords: ['earnings', 'revenue', 'growth'] },
            { name: 'Fed Policy', weight: 0.18, keywords: ['federal', 'interest', 'rate'] },
            { name: 'AI/Innovation', weight: 0.15, keywords: ['artificial', 'intelligence', 'innovation'] },
            { name: 'Geopolitical', weight: 0.12, keywords: ['trade', 'geopolitical', 'sanctions'] },
            { name: 'Crypto/Blockchain', weight: 0.05, keywords: ['bitcoin', 'crypto', 'blockchain'] }
        ];
    ...
    renderAttentionWeightsChart() {
        ...
        const words = ['The', 'stock', 'market', 'showed', 'strong', 'performance', 'despite', 'economic', 'uncertainty'];
        const attentionData = [0.05, 0.18, 0.22, 0.08, 0.15, 0.25, 0.12, 0.20, 0.18];
            ...
            children: [
                {
                    ...
                    children: [
                        ...
                        { name: 'No: News Sentiment > 0.2?', value: 'Normal RSI',
                          children: [
    ...
    renderGradientAttributionChart() {
        ...
        const features = ['Price', 'Volume', 'RSI', 'MACD', 'News_Sentiment', 'VIX', 'SP500_Corr'];
    ...
    renderIntegratedGradientsChart() {
        ...
        for (let i = 0; i <= 50; i++) {
            steps.push(i / 50);
            integratedGradients.push(Math.sin(i * 0.1) * Math.exp(-i * 0.05) + Math.random() * 0.1);
        }
    ...
    renderDriftDetectionChart() {
        ...
        for (let i = 30; i >= 0; i--) {
            ...
            const driftScore = Math.abs(Math.sin(i * 0.3)) * 0.15 + Math.random() * 0.05;
    ...
    renderEnsembleVotingChart() {
        ...
        const models = ['Random Forest', 'Gradient Boosting', 'LSTM', 'Transformer'];
        const predictions = [
            { model: 'Random Forest', buy: 0.65, hold: 0.25, sell: 0.10 },
            { model: 'Gradient Boosting', buy: 0.72, hold: 0.18, sell: 0.10 },
            { model: 'LSTM', buy: 0.58, hold: 0.32, sell: 0.10 },
            { model: 'Transformer', buy: 0.80, hold: 0.15, sell: 0.05 }
        ];
...
// Legacy extension code - removed to prevent DashboardManager undefined error
// Extensions are now initialized directly in the DashboardManager class
