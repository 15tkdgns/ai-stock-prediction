...
class RealTimeNewsAnalyzer {
  constructor() {
    this.apiKey =
      localStorage.getItem('newsapi_key') || '21e8dba1ab28429f9fd3b3943977afb0'; // NewsAPI Key
    if (!this.apiKey) {
  ...
  async loadLlmEnhancedFeatures() {
    try {
      const response = await fetch(
        '../data/processed/llm_enhanced_features.csv'
      );
      const text = await response.text();
      const lines = text.split('\n');
      const headers = lines[0].split(',').map((header) => header.trim());

      this.llmFeatures = lines
        .slice(1)
      ...
      console.log(
        `LLM enhanced features loaded: ${this.llmFeatures.length} items.`
      );
    } catch (error) {
    ...
    try {
      ...
      results.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          ...
          console.warn(
            `News source ${index + 1} collection failed:`,
            result.reason
          );
        }
      });
      ...
      if (newsArticles.length > 0) {
        ...
        console.log(
          `${this.newsCache.length} news articles collected and analyzed.`
        );
  ...
  async analyzeNews(newsArticles) {
    ...
    for (const article of newsArticles) {
      try {
        ...
        let eventCategory = this.categorizeNews(
  ...
  calculateImportance(article, llmFeature) {
    // Add llmFeature argument
    ...
    if (llmFeature) {
      ...
      if (
        llmFeature.market_sentiment === 'Bullish' ||
        llmFeature.market_sentiment === 'Bearish'
      ) {
        score += 0.1; // Bullish or bearish news is more important
      }
      // Importance based on specific event category
      if (
        llmFeature.event_category === 'M&A' ||
        llmFeature.event_category === 'Financials'
      ) {
        score += 0.15;
      }
    }
    ...
    const fallbackNews = [
      {
        ...
        content:
          'Major stock indices showed an overall upward trend today, receiving positive reactions from investors.',
        sentiment: 'positive',
        sentimentScore: 0.6,
        source: 'Market Watch',
      ...
      },
      {
        ...
        content:
          'Recent remarks from Federal Reserve officials are drawing market attention, providing hints about future interest rate policy.',
        sentiment: 'neutral',
        sentimentScore: 0.1,
  ...
  generateNewsSummary() {
    ...
    const avgSentiment =
      this.newsCache.reduce((sum, news) => sum + news.sentimentScore, 0) /
