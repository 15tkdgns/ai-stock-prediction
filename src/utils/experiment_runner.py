...
class ExperimentRunner:
        ...
        try:
            ...
            experiment_result = {
                ...
                'data_info': {
                    ...
                    'selected_features': selected_features.tolist() if hasattr(selected_features, 'tolist') else list(selected_features),
                    'class_distribution': {
            ...
            self.logger.info(f"실험 {exp_id} 완료 - 정확도: {cv_scores.get('accuracy', {}).get('mean', 0):.4f}")
    ...
    def run_experiment_batch(self, experiment_plan_path, max_experiments=None):
        ...
        for i, experiment_config in enumerate(experiments):
            ...
            # 중간 결과 저장 (10개마다)
            if (i + 1) % 10 == 0:
                self.save_batch_results(results, experiment_plan['experiment_name'], partial=True)
    ...
    def save_batch_results(self, results, experiment_name, partial=False):
        ...
        # JSON 형식으로 저장
        results_file = f'{self.results_dir}/{experiment_name}_results{suffix}_{timestamp}.json'
        ...
        summary_file = f'{self.results_dir}/{experiment_name}_summary{suffix}_{timestamp}.json'
    ...
    def generate_results_summary(self, results):
            ...
            if metric in performance_df.columns:
                ...
                summary['best_performances'][metric] = {
                    ...
                    'configuration': {
                        ...
                        'feature_combination': performance_df.loc[best_idx, 'feature_combination'],
        ...
        if 'model' in performance_df.columns:
            for metric in ['accuracy', 'precision', 'recall', 'f1_score']:
                if metric in performance_df.columns:
                    model_performance = performance_df.groupby('model')[metric].agg(['mean', 'std']).round(4)
        ...
        if 'preprocessing' in performance_df.columns:
            for metric in ['accuracy', 'precision', 'recall', 'f1_score']:
                if metric in performance_df.columns:
                    prep_performance = performance_df.groupby('preprocessing')[metric].agg(['mean', 'std']).round(4)
                    summary['preprocessing_comparison'][metric] = prep_performance.to_dict()
        
        # 특성 조합별 비교
        if 'feature_combination' in performance_df.columns:
            for metric in ['accuracy', 'precision', 'recall', 'f1_score']:
                if metric in performance_df.columns:
                    feat_performance = performance_df.groupby('feature_combination')[metric].agg(['mean', 'std']).round(4)
                    summary['feature_combination_comparison'][metric] = feat_performance.to_dict()
    ...
    def generate_comparison_plots(self, results, experiment_name):
        ...
        if 'model' in df.columns and 'accuracy' in df.columns:
            model_performance = df.groupby('model')['accuracy'].mean().sort_values(ascending=False)
        ...
        if 'preprocessing' in df.columns and 'accuracy' in df.columns:
            prep_performance = df.groupby('preprocessing')['accuracy'].mean().sort_values(ascending=False)
        ...
        if 'feature_combination' in df.columns and 'accuracy' in df.columns:
            feat_performance = df.groupby('feature_combination')['accuracy'].mean().sort_values(ascending=False)
            axes[1, 0].bar(feat_performance.index, feat_performance.values)
            axes[1, 0].set_title('Feature Combination Performance Comparison (Accuracy)')
        ...
        if len(df) > 0:
            ...
            if available_metrics:
                ...
                for i, (_, row) in enumerate(top_5_experiments.iterrows()):
                    ...
                    axes[1, 1].bar(x + i * width, values, width, label=f"Exp {row['experiment_id']}")
        ...
        plt.tight_layout()
        plt.savefig(f'{self.results_dir}/{experiment_name}_comparison_plots.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        self.logger.info(f"비교 플롯 저장 완료: {self.results_dir}/{experiment_name}_comparison_plots.png")
    ...
    try:
        ...
        if 0 <= choice < len(experiment_plans):
            ...
            if os.path.exists(selected_plan):
                ...
                # 실험 실행
                results = runner.run_experiment_batch(selected_plan, max_experiments=10)  # 테스트용 10개만
