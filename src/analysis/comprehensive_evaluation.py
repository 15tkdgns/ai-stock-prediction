...
from scipy.stats import wilcoxon, friedmanchisquare, chi2_contingency
import warnings
...
class ComprehensiveEvaluator:
    ...
    def define_evaluation_metrics(self):
        ...
        return {
            ...
            'basic_classification': {
                ...
                'precision_weighted': {
                    'function': lambda y_true, y_pred: precision_score(y_true, y_pred, average='weighted', zero_division=0),
                    'description': 'Weighted precision',
                    'interpretation': 'Higher is better (0-1 range)'
                },
                'recall_weighted': {
                    'function': lambda y_true, y_pred: recall_score(y_true, y_pred, average='weighted', zero_division=0),
            ...
            'advanced_classification': {
                ...
                'roc_auc': {
                    'function': lambda y_true, y_pred_proba: roc_auc_score(y_true, y_pred_proba[:, 1]) if y_pred_proba.ndim > 1 else roc_auc_score(y_true, y_pred_proba),
    ...
    def calculate_information_ratio(self, y_true, y_pred, returns=None, benchmark_returns=None):
        """정보 비율 계산"""
        if returns is None:
            returns = np.random.normal(0, 0.02, len(y_true))
    ...
    def calculate_prediction_entropy(self, y_pred_proba):
        ...
        if y_pred_proba.ndim > 1:
            ...
            entropy = -(proba_0 * np.log(proba_0 + 1e-15) + proba_1 * np.log(proba_1 + 1e-15))
    ...
    def mcnemar_test(self, y_true, y_pred1, y_pred2):
        ...
        mcnemar_stat = (abs(model1_correct - model2_correct) - 1) ** 2 / (model1_correct + model2_correct)
    ...
    def evaluate_single_model(self, y_true, y_pred, y_pred_proba=None, returns=None):
        ...
        for category_name, metrics in self.evaluation_metrics.items():
            ...
            for metric_name, metric_info in metrics.items():
                try:
                    if category_name == 'advanced_classification' and metric_name == 'roc_auc':
        ...
        for category_name, metrics in self.evaluation_metrics.items():
            ...
            for metric_name in metrics.keys():
                ...
                for model_name, model_result in model_results.items():
                    if (category_name in model_result and 
                        metric_name in model_result[category_name] and
                        model_result[category_name][metric_name]['score'] is not None):
                        scores[model_name] = model_result[category_name][metric_name]['score']
                
                if scores:
                    # 메트릭에 따른 정렬 (높을수록 좋은 메트릭 vs 낮을수록 좋은 메트릭)
                    reverse_sort = metric_name not in ['log_loss', 'max_drawdown', 'calibration_error', 'prediction_entropy']
                    sorted_models = sorted(scores.items(), key=lambda x: x[1], reverse=reverse_sort)
                    
                    comparison_results['model_rankings'][category_name][metric_name] = {
                        'ranking': [{'model': model, 'score': score} for model, score in sorted_models],
        ...
        if len(model_results) >= 2:
            ...
            for i in range(len(model_names)):
                for j in range(i+1, len(model_names)):
                    ...
                    # 기본 메트릭들로 비교
                    try:
                        acc1 = model_results[model1]['basic_classification']['accuracy']['score']
                        acc2 = model_results[model2]['basic_classification']['accuracy']['score']
                        
                        if acc1 is not None and acc2 is not None:
                            # 단순 차이 비교 (실제로는 여러 fold의 결과가 있어야 함)
                            comparison_results['statistical_tests'][f'{model1}_vs_{model2}'] = {
            ...
            for category_name, metrics in model_result.items():
                ...
                for metric_name, metric_result in metrics.items():
                    if metric_result['score'] is not None:
                        summary['model_statistics'][model_name]['evaluated_metrics'] += 1
                        category_scores.append(metric_result['score'])
                    else:
                        summary['model_statistics'][model_name]['failed_metrics'] += 1
                
                if category_scores:
                    summary['model_statistics'][model_name]['average_scores'][category_name] = np.mean(category_scores)
    ...
    def generate_recommendations(self, model_results):
        ...
        for model_name, model_result in model_results.items():
            ...
            for category_name, metrics in model_result.items():
                for metric_name, metric_result in metrics.items():
                    if metric_result['score'] is not None:
                        # 정규화된 점수 (0-1 범위)
                        if metric_name in ['log_loss', 'max_drawdown', 'calibration_error']:
                            # 낮을수록 좋은 메트릭
        ...
        for category_name, metrics in comparison_results['model_rankings'].items():
            ...
            for metric_name, ranking in metrics.items():
                if ranking['ranking']:
                    ...
                    category_winners[best_model] = category_winners.get(best_model, 0) + 1
        ...
        if overall_scores:
            ...
            if np.std(score_values) < 0.05:
                recommendations['performance_insights'].append("모델 간 성능 차이가 미미합니다.")
            
            if max(score_values) < 0.7:
                recommendations['improvement_suggestions'].append("모든 모델의 성능이 낮습니다. 특성 엔지니어링이나 하이퍼파라미터 튜닝을 고려해보세요.")
            
            if max(score_values) - min(score_values) > 0.3:
                recommendations['performance_insights'].append("모델 간 성능 차이가 큽니다. 최고 성능 모델을 선택하는 것이 중요합니다.")
    ...
    def plot_model_comparison(self, model_results, output_dir):
        ...
        for i, metric in enumerate(metrics_to_plot):
            if i < len(axes):
                ...
                for model_name, model_result in model_results.items():
                    if ('basic_classification' in model_result and 
                        metric in model_result['basic_classification'] and
                        model_result['basic_classification'][metric]['score'] is not None):
                        scores[model_name] = model_result['basic_classification'][metric]['score']
...
if __name__ == "__main__":
    ...
    sample_results = {
        'RandomForest': {
            'basic_classification': {
                'accuracy': {'score': 0.85, 'description': 'Overall accuracy', 'interpretation': 'Higher is better'},
                'precision_macro': {'score': 0.82, 'description': 'Macro precision', 'interpretation': 'Higher is better'},
                'recall_macro': {'score': 0.78, 'description': 'Macro recall', 'interpretation': 'Higher is better'},
                'f1_macro': {'score': 0.80, 'description': 'Macro F1', 'interpretation': 'Higher is better'}
            }
        },
        'XGBoost': {
            'basic_classification': {
                'accuracy': {'score': 0.87, 'description': 'Overall accuracy', 'interpretation': 'Higher is better'},
                'precision_macro': {'score': 0.84, 'description': 'Macro precision', 'interpretation': 'Higher is better'},
                'recall_macro': {'score': 0.81, 'description': 'Macro recall', 'interpretation': 'Higher is better'},
                'f1_macro': {'score': 0.82, 'description': 'Macro F1', 'interpretation': 'Higher is better'}
            }
        }
    }
