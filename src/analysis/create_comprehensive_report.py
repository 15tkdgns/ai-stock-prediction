    ...
    for model_name, perf in training_perf.items():
        ...
        if model_name == realtime_results['model_used']:
            ...
            analysis['realtime_results'] = {
                ...
                'normal_predictions': sum(1 for r in rt_results if r['prediction']['prediction'] == 0),
                'event_predictions': sum(1 for r in rt_results if r['prediction']['prediction'] == 1)
            }
        else:
            analysis['realtime_results'] = {
                'tested': False,
...
def create_comprehensive_visualizations(model_analysis, data):
    ...
    train_scores = [model_analysis[m]['training_results']['train_accuracy'] for m in models]
    test_scores = [model_analysis[m]['training_results']['test_accuracy'] for m in models]
    
    x = np.arange(len(models))
    width = 0.35
    
    axes[0, 0].bar(x - width/2, train_scores, width, label='Train Accuracy', color='skyblue')
    axes[0, 0].bar(x + width/2, test_scores, width, label='Test Accuracy', color='lightcoral')
    ...
    for i, (train, test) in enumerate(zip(train_scores, test_scores)):
        axes[0, 0].text(i - width/2, train + 0.005, f'{train:.3f}', ha='center', va='bottom', fontsize=8)
        axes[0, 0].text(i + width/2, test + 0.005, f'{test:.3f}', ha='center', va='bottom', fontsize=8)
    
    # 2. 오버피팅 분석
    overfitting_gaps = [model_analysis[m]['training_results']['overfitting_gap'] for m in models]
    colors = ['green' if gap <= 0.05 else 'orange' if gap <= 0.10 else 'red' for gap in overfitting_gaps]
    ...
    for i, gap in enumerate(overfitting_gaps):
        axes[0, 1].text(i, gap + 0.002, f'{gap:.3f}', ha='center', va='bottom', fontsize=8)
    
    # 3. 모델별 성능 등급
    training_grades = [model_analysis[m]['overall_assessment']['training_grade'] for m in models]
    overfitting_grades = [model_analysis[m]['overall_assessment']['overfitting_grade'] for m in models]
    ...
    axes[0, 2].bar(x - width/2, training_nums, width, label='Training Grade', color='lightblue')
    axes[0, 2].bar(x + width/2, overfitting_nums, width, label='Overfitting Grade', color='lightgreen')
    ...
    for i, (t_grade, o_grade) in enumerate(zip(training_grades, overfitting_grades)):
        axes[0, 2].text(i - width/2, training_nums[i] + 0.1, t_grade, ha='center', va='bottom', fontsize=10, fontweight='bold')
        axes[0, 2].text(i + width/2, overfitting_nums[i] + 0.1, o_grade, ha='center', va='bottom', fontsize=10, fontweight='bold')
    ...
    if gb_realtime['tested']:
        ...
        axes[1, 0].bar(tickers, event_probs, color='orange', alpha=0.7, label='Event Probability')
        ...
        for i, prob in enumerate(event_probs):
            axes[1, 0].text(i, prob + 0.0001, f'{prob:.4f}%', ha='center', va='bottom', fontsize=8)
    else:
        axes[1, 0].text(0.5, 0.5, 'No Real-time Test Data', ha='center', va='center', transform=axes[1, 0].transAxes)
    ...
    if gb_realtime['tested']:
        ...
        for i, conf in enumerate(confidences):
            axes[1, 1].text(i, conf + 0.01, f'{conf:.2f}%', ha='center', va='bottom', fontsize=8)
    else:
        axes[1, 1].text(0.5, 0.5, 'No Real-time Test Data', ha='center', va='center', transform=axes[1, 1].transAxes)
    ...
    event_counts = [event_stats['price_events'], event_stats['volume_events'], event_stats['volatility_events']]
...
"""
    ...
    # 모델 순위 및 추천
    best_model = max(model_analysis.items(), key=lambda x: x[1]['training_results']['test_accuracy'])
    most_stable = min(model_analysis.items(), key=lambda x: x[1]['training_results']['overfitting_gap'])
...
def main():
    ...
    for model_name, analysis in model_analysis.items():
        ...
        print(f"    - 실시간 테스트: {'완료' if analysis['realtime_results']['tested'] else '미완료'}")
