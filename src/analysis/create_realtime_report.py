...
def generate_market_outlook(ticker_results):
    """시장 전망 생성"""
    high_risk_stocks = [
        t for t, r in ticker_results.items() if r["risk_level"] == "HIGH"
    ]
    medium_risk_stocks = [
        t for t, r in ticker_results.items() if r["risk_level"] == "MEDIUM"
    ]
...
def create_visualizations(results, analysis):
    ...
    event_probs = [
        r["prediction"]["event_probability"] * 100 for r in results["results"]
    ]
    ...
    bars = axes[0, 0].bar(
        ...
        event_probs,
        color=[
            "red" if p > 65 else "orange" if p > 50 else "green" for p in event_probs
        ],
    )
    ...
    wedges, texts, autotexts = axes[1, 0].pie(
        risk_counts, labels=risk_levels, colors=colors, autopct="%1.0f%%", startangle=90
    )
    ...
    for i, (ticker, conf) in enumerate(zip(tickers, confidences)):
        axes[1, 1].text(
            i, conf + 1, f"{conf:.1f}%", ha="center", va="bottom", fontsize=8
        )

    plt.tight_layout()
    plt.savefig(
        "raw_data/realtime_test_visualization.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
