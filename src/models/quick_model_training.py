...
def create_sample_data():
    ...
    for ticker in tickers:
        ...
        try:
            ...
            if not hist.empty:
                ...
                hist['obv'] = (hist['volume'] * np.where(hist['close'] > hist['close'].shift(1), 1, -1)).cumsum()
                
                # 추가 특성
                hist['unusual_volume'] = (hist['volume'] > hist['volume'].rolling(window=20).mean() * 2).astype(int)
    ...
    combined_data['volume_event'] = (combined_data['volume'] > combined_data['volume'].rolling(window=30).mean() * 3).astype(int)
    combined_data['volatility_event'] = (combined_data['volatility'] > combined_data['volatility'].quantile(0.9)).astype(int)
    combined_data['major_event'] = ((abs(combined_data['price_event']) == 1) | 
                                   (combined_data['volume_event'] == 1) | 
                                   (combined_data['volatility_event'] == 1)).astype(int)
    combined_data['event_score'] = abs(combined_data['price_event']) + combined_data['volume_event'] + combined_data['volatility_event']
    ...
    label_columns = [
        'ticker', 'date', 'price_event', 'volume_event', 'volatility_event', 'major_event', 'event_score'
    ]
    ...
    for _, row in combined_data.iterrows():
        if np.random.random() < 0.3:  # 30% 확률로 뉴스 있음
            news_data.append({
                ...
                'source': 'Sample News',
                'sentiment_label': 'positive' if row['news_sentiment'] > 0.6 else 'negative' if row['news_sentiment'] < 0.4 else 'neutral',
