...
class RealTimeTestingSystem:
    ...
    def collect_realtime_data(self, ticker, period='1d', interval='1m'):
        ...
        try:
            ...
            return self._calculate_features(ticker, hist, avg_sentiment, avg_polarity, news_count)
    ...
    def _calculate_features(self, ticker, hist, avg_sentiment, avg_polarity, news_count):
    ...
    def evaluate_prediction_performance(self, ticker, actual_event=None):
        ...
        try:
            ...
            # 최근 예측 가져오기
            recent_predictions = [p for p in self.prediction_history if p['ticker'] == ticker]
            ...
            if actual_event is None:
                # 5% 이상 가격 변동을 이벤트로 간주
                current_price = latest_pred['features']['close']
                # 여기서는 실제 검증 로직 구현 필요
                actual_event = 0  # 임시값
            ...
            try:
                for ticker in tickers:
                    ...
                    if features:
                        ...
                        # 성능 평가
                        performance = self.evaluate_prediction_performance(ticker)
                        
                        # 결과 저장
                        with open(f'{self.data_dir}/realtime_results.json', 'w') as f:
                            json.dump({
                                'latest_predictions': list(self.prediction_history)[-100:],
